...
export class UpdateScoreOnGoalEventCommand {
    ...
    async execute(player: SheetPlayer, match: IMatch, newGoals: number): Promise<void> {
        const matchSheetRef = this.firestore.collection<MatchSheet>(this.matchSheetsCollectionName).doc(match.id);
        ...
        await this.firestore.firestore.runTransaction(async (transaction) => {
            ...
            const playerIndex = matchSheet.players.findIndex(p => p.playerId === player.id && p.team === player.equipo);
