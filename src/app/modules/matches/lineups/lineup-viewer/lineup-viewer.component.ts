...
export class LineupViewerComponent extends LineupBase implements OnInit {
  ...
  getFormation(lineup: LineUp[], home: boolean) {
    const g = this.group<LineUp>(
      'playerType',
      lineup.filter((p) => p.startging),
      (a, b) => a.order - b.order
    );
    if (!home) {
      return g.reverse();
  ...
  ngOnInit(): void {
    super.loadLineups(this.local, this.visita).then(() => {
      this.homeFormation = this.getFormation(this.localLineup, true);
      this.awayFormation = this.getFormation(this.visitaLineup, false);
  ...
  async loadPlayerImages() {
    const localPlayerObservables = this.localLineup.map((l) =>
      this.playerService.getBadge(l as unknown as IPlayer)
    );
    const visitaPlayerObservables = this.visitaLineup.map((l) =>
      this.playerService.getBadge(l as unknown as IPlayer)
    );

    if (localPlayerObservables.length > 0) {
      await firstValueFrom(
        forkJoin(localPlayerObservables).pipe(
          tap((badges) => {
    ...
    if (visitaPlayerObservables.length > 0) {
      await firstValueFrom(
        forkJoin(visitaPlayerObservables).pipe(
          tap((badges) => {
            // Assign the image URLs to the corresponding models
  ...
  group<T>(key: string, dataArray: T[], sort?: (a: T, b: T) => number) {
    const grouped = dataArray.reduce(
      (groupedData: { [key: string]: T[] }, data: T) => {
    ...
    return Object.entries(grouped).map(([key, value]) => ({
      key,
      values: sort ? value.sort(sort) : value,
    }));
  }
}
